...
import { Story, StoryStatus, StoryType, StoryPriority } from './entities/story.entity'
...
export class StoriesService {
  ...
  async create(createStoryDto: CreateStoryDto, userId: string, userRole?: string): Promise<Story> {
    ...
    try {
      ...
      if (categoryIds && categoryIds.length > 0) {
        ...
        requestLogger.debug({ categoriesAttached: categories.length }, 'Categories attached to story')
      }
  ...
  async findChildren(
    ...
    this.logger.debug({ action: 'find_children', parentId, includeDeleted }, 'Finding child stories')
  ...
  async remove(id: number, userId: string, reason?: string, userRole?: string): Promise<void> {
    ...
    try {
      ...
      this.logger.info({ action: 'delete_story', storyId: id, userId }, 'Story deleted successfully')
    } catch (error) {
      this.logger.error(
  ...
  async hardDelete(id: number, userId: string, userRole?: string): Promise<void> {
    this.logger.warn({ action: 'hard_delete_story', storyId: id, userId }, 'Permanently deleting story')
  ...
  async restore(id: number, userId: string, userRole?: string): Promise<Story> {
    ...
    try {
      ...
      this.logger.info({ action: 'restore_story', storyId: id, userId }, 'Story restored successfully')
  ...
  async updateStatus(
    ...
    this.logger.info({ action: 'update_status', storyId: id, userId, status }, 'Updating story status')
    ...
    try {
      ...
      this.logger.info(
        { action: 'update_status', storyId: id, userId, oldStatus: story.status, newStatus: status },
  ...
  async detachAttachments(
    ...
    try {
      ...
      story.attachments = story.attachments?.filter(
        (a) => !dto.attachmentIds.includes(a.id),
      ) || []
      story.updatedBy = userId
  ...
  async detachCategories(
    ...
    try {
      ...
      story.categories = story.categories?.filter(
        (c) => !dto.categoryIds.includes(c.id),
      ) || []
      story.updatedBy = userId
  ...
  private async resolveAndCreateTags(
    ...
    if (tagIds && tagIds.length > 0) {
      ...
      try {
        ...
        this.logger.debug({ foundCount: foundTags.length, requestedCount: tagIds.length }, 'Tags found by ID');
    ...
    if (tagNames && tagNames.length > 0) {
      ...
      for (const tagName of tagNames) {
        ...
        try {
          ...
          this.logger.error({ error: tagError, tagName: normalizedName }, 'Error processing tag name');
          throw tagError;
        }
      }
    }

    this.logger.debug({ totalTags: resolvedTags.length, tags: resolvedTags.map(t => ({ id: t.id, name: t.name })) }, 'Tags resolution complete');
    return resolvedTags
  }
