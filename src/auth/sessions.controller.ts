...
export class SessionsController {
  ...
  async listSessions(
    ...
    if (filter.filterByDevice) {
      filtered = filtered.filter(s => s.deviceType?.toLowerCase() === filter.filterByDevice?.toLowerCase());
    }
    if (filter.filterByType) {
      filtered = filtered.filter(s => s.sessionType?.toLowerCase() === filter.filterByType?.toLowerCase());
    }
      ...
      paginated.map(async (session) => {
        ...
        if (filter.includeRiskAssessment) {
          try {
            ...
            this.logger.warn({ sessionId: session.id, error: error.message }, 'Risk assessment failed');
          }
        }
        return sessionData;
      }),
    ...
    this.logger.info({ userId, count: mapped.length, total: filtered.length, duration }, 'Sessions listed');
    
    return {
      pagination: { total: filtered.length, page, limit, totalPages: Math.ceil(filtered.length / limit) || 1 },
      sessions: mapped,
    };
  }
  ...
  async getSessionStats(@Req() req: AuthenticatedRequest) {
    ...
    this.logger.info({ userId, totalSessions: result.totalSessions, activeSessions: result.activeSessions }, 'Stats retrieved');
    return result;
  }
  ...
  async getSession(@Param('id') sessionId: string, @Req() req: AuthenticatedRequest) {
    ...
    if (validation.session.userId !== userId) {
      this.logger.warn({ userId, sessionId, ownerId: validation.session.userId }, 'Unauthorized access attempt');
  ...
  async revokeSession(
    ...
    const userId = this.getUserId(req);
    this.logger.info({ userId, sessionId, reason: dto.reason, action: 'revoke_session' }, 'Revoking session');
  ...
  async revokeAllSessions(@Body() dto: RevokeSessionDto, @Req() req: AuthenticatedRequest) {
    ...
    const count = await this.sessionsService.revokeAllSessions(userId, undefined, dto.reason || 'user_logout_all');
  ...
  async revokeByDeviceType(
    ...
    for (const session of matching) {
      const success = await this.sessionsService.revokeSession(session.id, dto.reason || 'device_change');
      if (success) {
        revokedCount++;
  ...
  async extendSession(
    ...
    this.logger.info({ userId, sessionId, newExpiresAt: newExpiresAt.toISOString() }, 'Session extended');
  ...
  @ApiResponse({ status: 200 })
  async manualCleanup(@Query('olderThanDays') olderThanDays: number = 30, @Req() req: AuthenticatedRequest) {
    const user = req.user;
    ...
    this.logger.info({ adminUserId: user.userId, olderThanDays, action: 'manual_cleanup' }, 'Starting cleanup');
