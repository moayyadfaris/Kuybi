...
export class EmailTemplateService {
  private templateCache: Map<EmailTemplate, HandlebarsTemplateDelegate> = new Map()
  ...
  private loadTemplate(template: EmailTemplate): HandlebarsTemplateDelegate {
    try {
      ...
      this.logger.debug({ template, path: templatePath }, 'Email template loaded and cached')

      return compiledTemplate
    } catch (error) {
      this.logger.error({ template, error: error.message }, 'Failed to load email template')
      throw error
    }
  }
   ...
   */
  async render(template: EmailTemplate, context: EmailTemplateContext): Promise<string> {
    try {
      ...
      this.logger.error({ template, error: error.message }, 'Failed to render email template')
      throw error
    }
  }
  ...
  async preloadTemplates(): Promise<void> {
    ...
    for (const template of templates) {
      try {
        ...
        this.logger.warn({ template, error: error.message }, 'Failed to preload template')
      }
    }

    this.logger.info({ count: this.templateCache.size }, 'Email templates preloaded')
  }
}
