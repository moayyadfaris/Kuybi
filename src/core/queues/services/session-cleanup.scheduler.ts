...
export class SessionCleanupScheduler implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.ensureRepeatableJob('session-cleanup-hourly', SessionCleanupJobType.CLEANUP_EXPIRED, {
      pattern: '0 * * * *',
    })

    await this.ensureRepeatableJob('session-expiring-check', SessionCleanupJobType.CHECK_EXPIRING, {
      pattern: '*/30 * * * *',
    }, { withinMinutes: 60 })
  }
